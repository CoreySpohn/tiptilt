...
def test_internal_vocabulary_fails(tmp_path):
    bad = tmp_path / "vocab.py"
    content = '"""The Tier-G speckle product."""\n'  # internal-ref-ok
    bad.write_text(content)
    result = run_guard(bad)
    assert result.returncode == 1
...
def test_waiver_comment_allows(tmp_path):
    ...
    waived.write_text(
        'DATA = "hwo-mission-control/some/data/path"  # internal-ref-ok\n'
    )
